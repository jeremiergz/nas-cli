...
- **Command pattern**: Each CLI feature lives under `internal/cmd/<name>` and exposes `New() *cobra.Command`. Example: [internal/cmd/info/info.go](../internal/cmd/info/info.go) implements flags, subcommands, and helpers. Follow the same `New()` factory signature and register with `main.go`.
...
- **Command pattern**: Each CLI feature lives under `internal/cmd/<name>` and exposes `New() *cobra.Command`. Example: `internal/cmd/info/info.go` implements flags, subcommands, and helpers. Follow the same `New()` factory signature and register with `main.go`.
