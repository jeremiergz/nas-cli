			...
			if len(tvShows) == 0 {
				...
				if !dryRun {
					...
					if err != nil {
						...
						hasError := false
						ok, results := processTVShows(cmd.Context(), w, config.WD, tvShows)
						if !ok {
							hasError = true
						}
...
func processTVShows(ctx context.Context, w io.Writer, wd string, tvShows []*model.TVShow) (bool, []util.Result) {
	mkvSvc := ctxutil.Singleton[*mkvservice.Service](ctx)
